...
async def play_dice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
	...
	# await sleep(4)
	message_body = message_body.format(user_throw.value, bot_throw.dice.value)

	await context.bot.send_message(
		update.effective_message.chat_id,
		text="||" + escape_markdown(message_body, version=2) + "||",  # Nasconde il risultato dietro uno spoiler.
